...
VOID DirectX12Instance::CreateRTVBuffers()
{
    ...
    for (UINT frame = 0; frame < FRAMES; frame++) {
        ...
        m_hresult = swap_chain->GetBuffer(frame, IID_PPV_ARGS(&buffer));
        CheckSucceeded(m_hresult);
        ...
        m_hresult = device->CreateCommandAllocator(m_graphics_command_queue_desc.Type, IID_PPV_ARGS(&command_allocators[frame]));
        CheckSucceeded(m_hresult);
    }
