...
struct ENGINE_API SphereMesh
{
	...
	void GenerateSphere()
	{
		...
		// Center Vertices
		sphere[0] = Vertex(XMFLOAT3(0.0f, 2.0f, 0.0f), XMFLOAT4(Colors::AntiqueWhite), XMFLOAT2(0, 0));
		...
		for (int slice = m_PartCount; slice >= m_PartCount / 2; slice--)
		{
			...
			for (int stack = 0; stack <= m_PartCount; stack++)
			{
				...
				sphere[index++] = Vertex(XMFLOAT3(x, y, z), XMFLOAT4(Colors::LightSkyBlue), XMFLOAT2(0,0));
			}
		}

		// Deuxi�me demi-sph�re (inverse)
		for (int slice = 0; slice <= m_PartCount / 2 + 1; slice++)
		{
			...
			for (int stack = 0; stack <= m_PartCount; stack++)
			{
				...
				// Calcul des coordonn�es invers�es par rapport � la premi�re demi-sph�re
				x = radius * sin(phi) * cos(theta);
				y = -radius * sin(phi) * sin(theta);
				z = radius * cos(phi); // Inversion de z

				sphere[index++] = Vertex(XMFLOAT3(x, y, z), XMFLOAT4(Colors::Beige), XMFLOAT2(0,0));
			}
		}
